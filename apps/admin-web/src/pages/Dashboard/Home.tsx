...
const StatCard: React.FC<StatCardProps> = ({
  ...
  <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3 p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
    <div className={`${accent} rounded-xl p-3 shrink-0`}>{icon}</div>
    <div className="flex-1 min-w-0">
      ...
      {trend && (
        <div
          className={`flex items-center gap-1 mt-1 text-xs font-medium ${trend.up ? 'text-emerald-600' : 'text-red-500'}`}
        >
          <svg
            className="w-3 h-3"
            fill="none"
...
const RoleBadge: React.FC<{ role: string }> = ({ role }) => {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${map[role] ?? map.CITIZEN}`}
    >
      {labels[role] ?? role}
    </span>
  );
...
const StatusDot: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className="flex items-center gap-1.5 text-xs">
      <span
        className={`w-2 h-2 rounded-full ${s === 'ACTIVE' ? 'bg-emerald-500' : s === 'INACTIVE' ? 'bg-amber-400' : 'bg-red-400'}`}
      />
      <span
        className={
          s === 'ACTIVE'
                ...
                {[
                  {
                    label: 'Manage Users',
                    href: '/users',
                    icon: 'M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6-4.13a4 4 0 11-8 0 4 4 0 018 0z',
                    color:
                  ...
                  >
                    ...
                    <svg
                      className="w-4 h-4 text-gray-300 ml-auto group-hover:text-gray-500 transition-colors"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
