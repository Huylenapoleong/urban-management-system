...
const Regions: React.FC = () => {
  ...
  useEffect(() => {
    if (searchTerm.trim() === "") {
      ...
      const filtered = filterAndExpandRegions(allRegions, searchTerm.toLowerCase());
      setRegions(filtered);
    }
  ...
  const filterAndExpandRegions = (items: RegionWithExpanded[], searchTerm: string): RegionWithExpanded[] => {
    return items.reduce((acc: RegionWithExpanded[], region) => {
      const matches = region.name.toLowerCase().includes(searchTerm);
      const hasMatchingChildren = region.children && region.children.some(child => 
        filterAndExpandRegions([child], searchTerm).length > 0
      );
      ...
      if (matches || hasMatchingChildren) {
        const filteredRegion = {
          ...
          children: region.children ? filterAndExpandRegions(region.children, searchTerm) : undefined
        };
  ...
  const renderTreeItem = (item: RegionWithExpanded, level: number = 0) => (
    <div key={item.id} style={{ marginLeft: `${level * 24}px` }} className="py-2">
  ...
  return (
    <>
      <PageMeta title={t("regions.title")} description={t("regions.description")} />
