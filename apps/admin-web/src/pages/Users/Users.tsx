...
const USER_ROLES = [
  ...
  'PROVINCE_OFFICER',
  'ADMIN',
] as const satisfies readonly CreateUserRequest['role'][];
const USER_STATUSES = [
  'ACTIVE',
  'INACTIVE',
  'DEACTIVATED',
] as const satisfies readonly NonNullable<UpdateUserRequest['status']>[];

function normalizeUserRole(role: string): CreateUserRequest['role'] {
  const normalizedRole = role.toUpperCase();
  return USER_ROLES.find((item) => item === normalizedRole) ?? 'CITIZEN';
}

function normalizeUserStatus(
  status: string,
): NonNullable<UpdateUserRequest['status']> {
  const normalizedStatus = status.toUpperCase();
  return USER_STATUSES.find((item) => item === normalizedStatus) ?? 'ACTIVE';
}
...
type UserFormState = {
  ...
  status: NonNullable<UpdateUserRequest['status']>;
  locationCode: string;
  unit: string;
};
  ...
  <select
    ...
    className={`w-full px-3.5 py-2.5 text-sm rounded-xl border transition-all focus:outline-none focus:ring-2 ${
      disabled
        ...
        : error
          ? 'border-red-300 focus:ring-red-100 bg-red-50 dark:bg-red-900/10 dark:border-red-700'
          : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-gray-100 focus:ring-blue-100 focus:border-blue-400 dark:focus:border-blue-500'
    }`}
  >
    {children}
  </select>
...
const Users: React.FC = () => {
  ...
  const locationScope = isWardScopedRole(form.role) ? 'WARD' : 'PROVINCE';
  ...
  const buildLocationCode = (
    ...
    return isWardScopedRole(role)
      ? wardCode
        ? buildWardLocationCode(provinceCode, wardCode)
        : ''
      : buildProvinceLocationCode(provinceCode);
  };
  ...
  const validate = () => {
    ...
    if (!editingUser) {
      ...
      if (form.phone && !/^0\d{9}$/.test(form.phone.trim()))
        e.phone = '10 digits starting with 0';
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingUser) {
        const res = await usersService.changeUserStatus(
          editingUser.id,
          form.status,
        );
        if (res.success && res.data) {
          setUsers((u) =>
            u.map((x) => (x.id === editingUser.id ? { ...x, ...res.data } : x)),
          );
          closeModal();
          ...
          {[
            ...
            },
            {
              ...
              value: users.filter((u) =>
                ['WARD_OFFICER', 'PROVINCE_OFFICER', 'ADMIN'].includes(
                  u.role?.toUpperCase(),
                ),
              ).length,
            ...
            {
              label: 'Citizens',
              value: users.filter((u) => u.role?.toUpperCase() === 'CITIZEN')
                .length,
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-9 h-9 rounded-full bg-linear-to-br ${avatarColor(user.id)} flex items-center justify-center text-white text-xs font-bold shrink-0`}
                            >
                        ...
                        <td className="px-5 py-3.5">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-medium ${role.color}`}
                          >
                            {role.label}
                          </span>
                        </td>
                        ...
                        <div className="mt-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 px-3 py-2">
                          ...
                          <p className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                            {selectedWard?.fullName ||
                              ...
                              'Select the required administrative scope'}
                          </p>
                        </div>
                        {locationError && (
                    ...
                    <div className="col-span-2">
                      <Field label="Role">
                        <Select
                          ...
                          onChange={(v) => {
                            const nextRole = v as typeof form.role;
                            setForm((current) => ({
                              ...current,
                              role: nextRole,
                              unit: '',
                      ...
                      <div className="col-span-2">
                        <Field label="Unit" required error={formErrors.unit}>
                          <TInput
                            ...
                            placeholder="e.g. Ward Police Unit 1"
              ...
              {editingUser && (
                <>
                  ...
                  <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
                    <div
                      className={`w-10 h-10 rounded-full bg-linear-to-br ${avatarColor(editingUser.id)} flex items-center justify-center text-white text-sm font-bold shrink-0`}
                    >
                    ...
                    <span
                      className={`ml-auto px-2.5 py-1 rounded-full text-xs font-medium shrink-0 ${getRoleConfig(editingUser.role).color}`}
                    >
                  ...
                  <Field label="Status" required>
                    <Select
                      ...
                      onChange={(v) =>
                        setForm({
                          ...form,
                          status: v as NonNullable<UpdateUserRequest['status']>,
                        })
                      }
