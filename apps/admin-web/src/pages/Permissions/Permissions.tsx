...
const Permissions: React.FC = () => {
  ...
  const togglePermission = (index: number, action: keyof Omit<Permission, 'resource'>) => {
  ...
  return (
    <>
      ...
      <ComponentCard title={t("permissions.rolePermissionMatrix")}>
        <div className="mb-6">
          <label className="block text-sm font-medium mb-2">{t("permissions.selectRole")}</label>
          <select
            value={role}
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="p-4 text-left font-semibold text-gray-700">Resource</th>
                <th className="p-4 text-center font-semibold text-gray-700">Read</th>
                <th className="p-4 text-center font-semibold text-gray-700">Write</th>
                <th className="p-4 text-center font-semibold text-gray-700">Delete</th>
                <th className="p-4 text-center font-semibold text-gray-700">Approve</th>
              </tr>
            </thead>
            <tbody>
              {permissions.map((perm, index) => (
                <tr key={index} className="border-b hover:bg-gray-50 transition-colors">
                  <td className="p-4 font-medium text-gray-900">{perm.resource}</td>
