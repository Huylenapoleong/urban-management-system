...
export default function NotificationDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 z-10 min-w-4.5 h-4.5 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center">
      ...
      <Dropdown
        ...
        className="absolute right-0 mt-4.25 flex w-90 flex-col rounded-2xl border border-gray-200 bg-white shadow-theme-lg dark:border-gray-800 dark:bg-gray-900 overflow-hidden"
      >
        {/* Header */}
        ...
        {/* List */}
        <ul className="flex flex-col overflow-y-auto max-h-95 custom-scrollbar divide-y divide-gray-50 dark:divide-gray-800/60">
          {notifications.length === 0 ? (
            ...
            notifications.map((n) => (
              <li key={n.id}>
                <button
                  ...
                  className={`w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-white/2 transition-colors ${
                    !n.read ? 'bg-blue-50/40 dark:bg-blue-900/10' : ''
                  }`}
                >
                  {/* Icon */}
                  <div
                    className={`w-8 h-8 rounded-full ${typeConfig[n.type].bg} flex items-center justify-center shrink-0 mt-0.5`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p
                        className={`text-sm font-medium truncate ${!n.read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}
                      >
                        {n.title}
                      </p>
                      {!n.read && (
