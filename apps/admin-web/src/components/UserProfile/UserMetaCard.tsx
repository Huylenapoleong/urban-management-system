      ...
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            {/* Avatar */}
            <div className="w-20 h-20 overflow-hidden border-2 border-blue-100 dark:border-blue-900/40 rounded-full flex-shrink-0 bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center">
              {user?.avatarUrl ? (
            ...
            <div className="order-3 xl:order-2">
              ...
              <div className="flex flex-col items-center gap-1 xl:flex-row xl:gap-3 xl:text-left">
                ...
                {user?.unit && (
                  <>
                    <div className="hidden h-3.5 w-px bg-gray-300 dark:bg-gray-700 xl:block" />
                    <p className="text-sm text-gray-500 dark:text-gray-400">{user.unit}</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border ${
                ...
                <span className={`w-1.5 h-1.5 rounded-full ${user?.status?.toUpperCase() === "ACTIVE" ? "bg-emerald-500" : "bg-red-500"}`} />
                {user?.status ?? "Unknown"}
              </span>
            </div>
          </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[500px] m-4">
        <div className="no-scrollbar relative w-full overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8">
          <div className="px-2 pr-14 mb-6">
            <h4 className="mb-1 text-xl font-semibold text-gray-800 dark:text-white/90">Edit Profile</h4>
            <p className="text-sm text-gray-500 dark:text-gray-400">Update your display information.</p>
          </div>
