...
export default function UserMetaCard() {
  ...
  const { locationMap } = useResolvedLocations([user?.locationCode]);
  ...
  return (
    <>
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            {/* Avatar */}
            <div className="w-20 h-20 overflow-hidden border-2 border-blue-100 dark:border-blue-900/40 rounded-full shrink-0 bg-linear-to-br from-blue-400 to-blue-600 flex items-center justify-center">
              {user?.avatarUrl ? (
                <img
                  src={user.avatarUrl}
                  alt={user.name}
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${user?.status?.toUpperCase() === 'ACTIVE' ? 'bg-emerald-500' : 'bg-red-500'}`}
                />
                {user?.status ?? 'Unknown'}
              </span>
            </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-125 m-4">
