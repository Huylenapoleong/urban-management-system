...
export default function ReportDetailsScreen() {
  ...
  const { mutate: updateStatus, isPending: isUpdating } =
    useUpdateReportStatus();
  const { mutate: assignOfficer } = useAssignOfficer();
  ...
  const { data: officers, isLoading: isSearchingOfficers } =
    useUserSearch(officerSearchQuery);
  ...
  if (isLoading) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <Appbar.Header>
          <Appbar.BackAction
            onPress={() => router.push("/(official)/reports")}
          />
        ...
        <View style={styles.actionContainer}>
          ...
          {report.assignedOfficerId && (
            <View style={styles.assignedSection}>
              <Text variant="bodySmall" style={styles.assignedLabel}>
                Can bo dang xu ly:
              </Text>
              <Text variant="bodyMedium" style={styles.officerName}>
                ID: {report.assignedOfficerId}
              </Text>
            </View>
          )}
          ...
          <Button
            ...
            onPress={() => {
              if (report.userId) {
                // Fallback/Prioritize private DM with the citizen
                ...
                const targetId =
                  ...
                  ((report as any)?.groupId
                    ? `group:${(report as any).groupId}`
                    : undefined);
                if (targetId) {
                  void prefetchConversationMessages(queryClient, targetId);
                  router.push({
                  ...
                  Alert.alert(
                    "Thông báo",
                    "Báo cáo này chưa có phòng trao đổi liên kết.",
                  );
                }
              }
            }}
        ...
        <Portal>
          ...
          >
            ...
            <Dialog.Actions>
              ...
              <Button onPress={handleUpdateStatus} disabled={isUpdating}>
                Lưu
              </Button>
            </Dialog.Actions>
          </Dialog>
          ...
          >
                    ...
                    <List.Item
                      ...
                      left={(props: any) => (
                        <Avatar.Text
                          {...props}
                          size={32}
                          label={officer.fullName.substring(0, 2).toUpperCase()}
                        />
                      )}
                    />
                  ))
            ...
            <Dialog.Actions>
              <Button onPress={() => setAssignDialogVisible(false)}>
                Dong
              </Button>
            </Dialog.Actions>
          </Dialog>
