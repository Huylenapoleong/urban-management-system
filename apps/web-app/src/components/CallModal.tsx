...
function RemotePeerVideo({
  ...
  useEffect(() => {
    if (speakingUpdateTimerRef.current !== null) {
      window.clearTimeout(speakingUpdateTimerRef.current);
      speakingUpdateTimerRef.current = null;
    }

    const delay = isSpeaking ? 250 : 700;
    speakingUpdateTimerRef.current = window.setTimeout(() => {
    ...
    >
      ...
      {hasVideoTrack ? (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
          <Avatar
            className={`h-16 w-16 border border-slate-700 bg-slate-800 ${isSpeaking ? "ring-4 ring-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.3)]" : ""}`}
          >
            {fallbackAvatar && (
...
export function CallModal({ rtc }: CallModalProps) {
  ...
  // Layout calculations
  const totalVideos = remoteEntries.length;
  const isGridLayout = isGroup && totalVideos > 1;
  const remoteEntryIndexMap = new Map(
    remoteEntries.map(([peerId], index) => [peerId, index]),
  );

  const orderedRemoteEntries = isGridLayout
    ? [...remoteEntries].sort((left, right) => {
        const leftSpeaking = speakingPeers[left[0]] ? 1 : 0;
        const rightSpeaking = speakingPeers[right[0]] ? 1 : 0;
        ...
        return (
          (remoteEntryIndexMap.get(left[0]) ?? 0) -
          (remoteEntryIndexMap.get(right[0]) ?? 0)
        );
      })
    : remoteEntries;

  let gridCols = "grid-cols-1";
  const gridRows = "auto-rows-[minmax(180px,1fr)]";
  if (isGridLayout) {
    if (totalVideos === 2) gridCols = "grid-cols-2";
    else if (totalVideos <= 4) gridCols = "grid-cols-2";
    else gridCols = "grid-cols-3";
  }

  const modalWidth = isGridLayout
    ? "w-[90vw] max-w-[800px]"
    : "w-[350px] sm:w-[380px] max-w-[calc(100vw-20px)]";
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-slate-100">
              {title} {isGroup ? "nhóm" : `với ${peerName}`}
            </p>
        ...
        {orderedRemoteEntries.map(([peerId, stream]) => (
          ...
          >
            <RemotePeerVideo
              ...
              fallbackAvatar={
                !isGroup ? activeConfig?.peerAvatarUrl : undefined
              }
            />
          </div>
        ))}
      ...
      <div className="flex items-center justify-center gap-3 border-t border-slate-700/80 bg-slate-900/95 p-3.5">
        {callState === "INCOMING" ? (
          <>
            <Button
              variant="secondary"
              className="h-10 rounded-full border border-slate-600 bg-slate-700/60 px-4 hover:bg-slate-600 text-white"
              onClick={rejectCall}
            >
            ...
            <Button
              variant="default"
              className="h-10 rounded-full bg-emerald-600 px-4 hover:bg-emerald-700 text-white"
              onClick={acceptCall}
            >
          ...
          <>
            ...
            <Button
              variant="destructive"
              size="icon"
              className="mx-1 h-11 w-11 rounded-full bg-red-600 hover:bg-red-700 text-white"
              onClick={endCall}
            >
              <PhoneOff size={24} />
            </Button>
