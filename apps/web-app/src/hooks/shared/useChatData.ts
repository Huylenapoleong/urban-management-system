...
export function useConversationList(searchTerm?: string, matchIds?: string[]) {
  return useQuery({
    queryKey: ["conversations", searchTerm?.trim() ?? "", matchIds?.join(",") ?? ""],
    queryFn: async () =>
      normalizeConversationList(await listConversations(searchTerm, matchIds)),
    staleTime: 10 * 1000,
...
export function useConversations(searchTerm?: string, matchIds?: string[]) {
  return useConversationList(searchTerm, matchIds);
}
