...
import { CheckCircle, Clock, KanbanSquare, Loader2 } from "lucide-react";
import { toast } from "react-hot-toast";

type DashboardReport = Awaited<ReturnType<typeof getAllReports>>[number];
...
export default function OfficialDashboard() {
  ...
  const reportLocationCodes =
    reports?.map((report: DashboardReport) => report.locationCode) ?? [];
  const { data: locationMap } = useResolvedLocations(reportLocationCodes);
  ...
  const pending =
    reports?.filter((report: DashboardReport) => report.status === "PENDING") ||
    [];
  const processing =
    reports?.filter(
      (report: DashboardReport) => report.status === "PROCESSING",
    ) || [];
  const resolved =
    reports?.filter(
      (report: DashboardReport) => report.status === "RESOLVED",
    ) || [];
    ...
    <div className="container mx-auto p-4 max-w-6xl space-y-6">
      <header className="mb-8">
        ...
        <p className="text-gray-500 mt-2">
          Tổng quan các phản ánh và báo cáo từ người dân.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-amber-50 dark:bg-amber-900/20 p-6 rounded-xl border border-amber-200 dark:border-amber-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-amber-600 dark:text-amber-400 uppercase">
                Chờ xử lý
              </p>
              ...
              {reports?.map((report: DashboardReport) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-xs text-gray-600 dark:text-gray-300">
                    {getResolvedLocationLabel(locationMap, report.locationCode)}
                  </td>
                  <td className="px-6 py-4">
                    <span
