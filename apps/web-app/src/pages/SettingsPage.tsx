...
export function SettingsPage() {
  ...
  const [formDataEdits, setFormDataEdits] = useState<
    Partial<{
      fullName: string;
      phone: string;
      email: string;
  ...
  const { data: avatarHistory = [], isLoading: isLoadingAvatarHistory } =
    useQuery({
  ...
  const formData = {
    fullName: formDataEdits.fullName ?? profile?.fullName ?? "",
    phone: formDataEdits.phone ?? profile?.phone ?? "",
    email: formDataEdits.email ?? profile?.email ?? "",
  };
  ...
  const profileMutation = useMutation({
    ...
    onError: (err: { message?: string }) => {
      setProfileMessage({
        text: err?.message || "Lỗi khi cập nhật hồ sơ",
        type: "error",
      });
    },
  });
  ...
  const requestOtpMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPasswordMessage({
        text: "OTP đã được gửi, vui lòng kiểm tra email.",
        type: "success",
      });
    ...
    onError: (err: { message?: string }) => {
      setPasswordMessage({
        text: err?.message || "Không thể gửi OTP đổi mật khẩu",
        type: "error",
      });
    },
  });
  ...
  const passwordMutation = useMutation({
    ...
    onError: (err: { message?: string }) => {
      setPasswordMessage({
        text: err?.message || "Lỗi khi đổi mật khẩu",
        type: "error",
      });
  ...
  const avatarMutation = useMutation({
    ...
    onError: (err: { message?: string }) => {
      setAvatarUploading(false);
      setProfileMessage({
        text: err?.message || "Lỗi khi cập nhật ảnh đại diện",
        type: "error",
      });
    },
  });
  ...
  const handleProfileSubmit = (e: React.FormEvent) => {
    ...
    if (normalizedFullName.length < 2) {
      setProfileMessage({
        text: "Họ và tên cần ít nhất 2 ký tự.",
        type: "error",
      });
      return;
    }
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (!otpCode.trim()) {
      setPasswordMessage({
        text: "Thiếu mã OTP, vui lòng nhập lại",
        type: "error",
      });
      setPasswordStep("otp");
      return;
    ...
    if (passwordData.currentPassword.trim().length < 8) {
      setPasswordMessage({
        text: "Mật khẩu hiện tại phải có ít nhất 8 ký tự.",
        type: "error",
      });
      return;
    }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1">
              <form onSubmit={handleProfileSubmit} className="space-y-4">
                {profileMessage.text && (
                  <div
                    className={`p-3 rounded-lg text-sm ${profileMessage.type === "error" ? "bg-red-50 text-red-500" : "bg-green-50 text-green-600"}`}
                  >
                    {profileMessage.text}
                  </div>
                )}
        ...
        <div className="p-6 md:p-8 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/40">
          ...
          {passwordMessage.text && !passwordModalOpen && (
            <div
              className={`mb-4 max-w-md p-3 rounded-lg text-sm ${passwordMessage.type === "error" ? "bg-red-50 text-red-500" : "bg-green-50 text-green-600"}`}
            >
          ...
          {passwordStep === "otp" ? (
            <form onSubmit={handleOtpStepSubmit} className="space-y-4">
              <DialogHeader>
                ...
                <DialogDescription>
                  Mã OTP đã được gửi đến {maskedEmail || "email đã đăng ký"}.
                  Nhập OTP để tiếp tục đổi mật khẩu.
                </DialogDescription>
              </DialogHeader>

              {passwordMessage.text && (
                <div
                  className={`p-3 rounded-lg text-sm ${passwordMessage.type === "error" ? "bg-red-50 text-red-500" : "bg-green-50 text-green-600"}`}
                >
                  {passwordMessage.text}
                </div>
              )}
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              <DialogHeader>
                ...
                <DialogDescription>
                  OTP đã nhập: {otpCode}. Vui lòng nhập mật khẩu hiện tại và mật
                  khẩu mới.
                </DialogDescription>
              </DialogHeader>
      ...
      <Dialog open={avatarModalOpen} onOpenChange={setAvatarModalOpen}>
        <DialogContent className="sm:max-w-3xl dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100">
          <DialogHeader>
            ...
            <DialogDescription>
              Chọn một ảnh đã tải lên trước đó hoặc tải lên ảnh mới để dùng làm
              avatar.
            </DialogDescription>
          </DialogHeader>
                    ...
                    >
                      ...
                      <div className="space-y-1 p-3">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          {item.isInUse
                            ? "Ảnh hiện có thể đang được dùng"
                            : "Nhấn để dùng ảnh này"}
                        </p>
                      </div>
                    </button>
                  );
